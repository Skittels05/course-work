document.addEventListener('DOMContentLoaded', function() {
  ...
  function findElements(attempt = 0) {
    return new Promise((resolve, reject) => {
      ...
      const essentialElementsFound = state.elements.toggleBtn && state.elements.modal;
      ...
      if (essentialElementsFound || attempt >= config.maxAttempts) {
        ...
        setTimeout(() => findElements(attempt + 1).then(resolve), config.retryDelay);
      }
    });
  }
  ...
  function setupEventListeners() {
    ...
    document.addEventListener('click', function(e) {
      if (state.elements.modal.classList.contains('show') && 
    ...
    if (state.elements.fontButtons) {
      state.elements.fontButtons.forEach(btn => {
        btn.addEventListener('click', function() {
          state.elements.fontButtons.forEach(b => b.classList.remove('active'));
    ...
    if (state.elements.colorButtons) {
      state.elements.colorButtons.forEach(btn => {
        btn.addEventListener('click', function() {
          state.elements.colorButtons.forEach(b => b.classList.remove('active'));
          this.classList.add('active');
    ...
    if (saved) {
      try {
        ...
        if (state.settings.fontSize && state.elements.fontButtons) {
          const activeFontBtn = Array.from(state.elements.fontButtons)
            .find(btn => btn.dataset.size === state.settings.fontSize);
          if (activeFontBtn) activeFontBtn.classList.add('active');
        }
        
        if (state.settings.colorScheme && state.elements.colorButtons) {
          const activeColorBtn = Array.from(state.elements.colorButtons)
            .find(btn => btn.dataset.scheme === state.settings.colorScheme);
  ...
  const newSize = state.settings.fontSize ? sizeMap[state.settings.fontSize] : '';
  ...
  console.log('Font size and wrapping applied');
}
  ...
  function handleImages() {
    ...
    if (state.settings.hideImages) {
      ...
      main.querySelectorAll('img:not([alt=""]):not(.a11y-processed)').forEach(img => {
        ...
        altContainer.innerHTML = `
          ...
          <div class="a11y-image-alt-text">${img.alt || 'No description available'}</div>
        `;
      ...
      main.querySelectorAll('.a11y-image-alt-container').forEach(container => {
  ...
  function resetSettings() {
    ...
    if (state.elements.fontButtons) {
      state.elements.fontButtons.forEach(btn => btn.classList.remove('active'));
      const mediumBtn = Array.from(state.elements.fontButtons)
        .find(btn => btn.dataset.size === 'medium');
      if (mediumBtn) mediumBtn.classList.add('active');
    }
    
    if (state.elements.colorButtons) {
      state.elements.colorButtons.forEach(btn => btn.classList.remove('active'));
      const defaultBtn = Array.from(state.elements.colorButtons)
        .find(btn => btn.dataset.scheme === 'default');
