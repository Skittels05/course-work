document.addEventListener('DOMContentLoaded', function() {
    ...
    const commonPasswords = [
        ...
        'password1', '12345678', '111111', '1234567', '123123'
    ];

    const nicknameSuffixes = ['', '_', '-', '.', 'X', 'Z', '007', '42', '99', '2024'];
    ...
    async function checkFieldAvailability(fieldName, value, errorElementId, errorMessage) {
        try {
            const response = await fetch(`${apiUrl}/users?${fieldName}=${encodeURIComponent(value)}`);
    ...
    async function validateUniqueFields() {
        ...
        const isEmailUnique = await checkFieldAvailability('email', email, 'emailError', 'This email is already registered');
        const isNicknameUnique = await checkFieldAvailability('nickname', nickname, 'nicknameError', 'This nickname is already taken');
        const isPhoneUnique = await checkFieldAvailability('phone', phone, 'phoneError', 'This phone number is already registered');
    ...
    async function validateForm() {
        ...
        if (!validateRequiredField('firstName', 'firstNameError', 'Enter your first name')) {
            isValid = false;
        }
        ...
        if (!validateRequiredField('nickname', 'nicknameError', 'Enter or generate a nickname')) {
            isValid = false;
        }
    ...
    function validatePhoneFormat() {
        ...
        if (!/^\+375(24|25|29|33|44)\d{7}$/.test(phone)) {
            showError('phoneError', 'Enter a valid Belarusian phone number (+375XXXXXXXXX)');
            return false;
        }
    ...
    function validatePassword() {
        ...
        if (!/[A-Z]/.test(password) || !/[a-z]/.test(password) || 
            ...
            showError('passwordError', 'Password must contain uppercase, lowercase, numbers and symbols');
            return false;
        }
    ...
    form.addEventListener('submit', async function(e) {
        ...
        const nickname = document.getElementById('nickname').value.trim();
        const isNicknameAvailable = await checkFieldAvailability('nickname', nickname, 'nicknameError', 'This nickname is already taken');
